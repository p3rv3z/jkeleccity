...
<div class="content-header">
  <div class="container-fluid">
    <div class="row mb-2">
      <div class="col-sm-6">
        <h4 class="m-0 text-dark">Sale Information</h4>
      </div>
      <div class="col-sm-6">
          ...
          <div class="row invoice-info mb-4">
            <div class="col-sm-4 invoice-col">
              ...
              <address>
                ...
                Email: {{ $appConfig->email ?? '018XXXXXX' }}
              </address>
            </div>
            <!-- /.col -->
            <div class="col-sm-4 invoice-col">
              ...
              <address>
                ...
                @empty(! $sale->user->nid )
                  National ID: {{ $sale->user->nid }}<br>
                @endempty
                @empty(! $sale->user->birth_certificate_no)
                  Birth ID: {{ $sale->user->birth_certificate_no }}<br>
                @endif
                Phone: (+880) {{ $sale->user->phone }}<br>
            ...
            <div class="col-sm-4 invoice-col">
              ...
              <b>Date:</b> {{ Carbon\Carbon::parse($sale->created_at)->format('d M Y') }}
            </div>
            <!-- /.col -->
          </div>
          <!-- /.row -->
          ...
          <div class="row">
            ...
            <div class="col-6">
              ...
              @if($sale->payment_type == 'card')
                <br>
                <b>Card Type: </b>{{ $sale->card_type }}<br>
                <b>Card No: </b>{{ $sale->card_no }}
              @endif
              @if($sale->payment_type == 'cheque')
                <br>
                <b>Bank Name: </b>{{ $sale->bank_name }}<br>
                <b>Bank No: </b>{{ $sale->bank_no }}
              @endif
            </div>
            <!-- /.col -->
            <div class="col-6">
                ...
                <table class="table">
                  <tbody>
                    ...
                    @if($sale->discount > 0 )
                      ...
                      <tr>
                        <th>Total</th>
                        <td>&#2547;{{ $sale->cost_amount - $sale->discount }}</td>
                      </tr>
                    @endif
                    <tr>
                      <th>Due</th>
                      <td>&#2547;{{ ( $sale->cost_amount - $sale->discount ) - $sale->paid_amount }}</td>
                    </tr>
                    <tr>
                      <th>Paid</th>
