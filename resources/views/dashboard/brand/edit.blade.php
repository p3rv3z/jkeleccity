...
<div class="content-header">
  <div class="container-fluid">
    <div class="row mb-2">
      <div class="col-sm-6">
        <h4 class="m-0 text-dark">Edit Brand</h4>
      </div>
      <div class="col-sm-6">
          ...
          <form action="{{ route('brand.update', $brand->id) }}" method="post">
            ...
            <div class="card-body">
              <div class="form-group">
                ...
                <input type="text" name="name" value="{{ old('name') ?? $brand->name }}" class="form-control form-control-sm" id="name" placeholder="Enter Name" autofocus required>
                @error ('name')
              ...
              <div class="form-group">
                ...
                <select name="status" id="status" value="{{ old('status') ?? $brand->status }}" class="form-control form-control-sm select2-withoutSearch" required>
                  <option selected="selected" value="1">Active</option>
                  <option value="0" {{ (old('status') ?? $brand->status) ? null : 'selected' }}>Inactive</option>
                </select>
                @error ('status')
