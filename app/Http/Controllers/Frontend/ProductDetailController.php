...
class ProductDetailController extends Controller
{
    ...
    public function __invoke($id)
    {
      ...
      $product->with(['brand', 'product_type', 'product_type.category', 'purchase' => function ($q) {
        $q->latest()->first();
      }])->get(['id', 'name', 'description', 'model_name', 'final_stock']);

      $related_products = Product::with(['purchase' => function ($q) {
        $q->latest()->first();
      }])
